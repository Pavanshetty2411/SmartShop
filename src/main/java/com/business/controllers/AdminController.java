...
public class AdminController {
	...
	public String seachHandler(@RequestParam("productName") String name, Model model) {
		List<Product> matchingProducts = this.productServices.searchProducts(name);

		if (matchingProducts.isEmpty()) {
			model.addAttribute("message", "SORRY...! No products found matching your search.");
		}
		model.addAttribute("products", matchingProducts);
		if (matchingProducts.size() == 1 && matchingProducts.get(0).getPname().equalsIgnoreCase(name)) {
			model.addAttribute("product", matchingProducts.get(0));
		}
	...
	@GetMapping("/updatingAdmin/{adminId}")
